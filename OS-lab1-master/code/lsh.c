        ...
        if (cmd.pgm->next == NULL && is_builtin_command(cmd.pgm->pgmlist[0])) {
          ...
          if (pid == 0) {
            ...
            if (cmd.pgm->next == NULL) {
              ...
              int err = execvp(cmd.pgm->pgmlist[0], cmd.pgm->pgmlist);
              if (err < 0) {
            ...
            pid_t pid = fork();
            if (pid == 0) {
              redirect_stdin(pipefd); // redirect the command's stdin to take in
                                      // pipe's read end
              if (cmd.rstdout != NULL) {
                redirect_stdout_to_file(cmd.rstdout); // if > operator
              }
              int err = execvp(next_pgm->pgmlist[0], next_pgm->pgmlist);
              if (err < 0) {
                perror("execvp failed with");
              }
            ...
            while (next_pgm != NULL &&
                   next_pgm->next != NULL) { // loop until not at leftmost stage
              int previous_pipe[2] = {
                  pipefd[0], pipefd[1]}; // save previous pipe before forking
              if (pipe(pipefd) == -1)
                err(EXIT_FAILURE, "pipe");

              pid_t pid = fork();
              if (pid == 0) {                   // child branch
                redirect_stdout(previous_pipe); // redirect command's stdout to
                                                // previous pipe's write end
                redirect_stdin(pipefd); // redirect command's stdin to take in
                                        // new pipe's read end
                int err = execvp(next_pgm->pgmlist[0], next_pgm->pgmlist);
                if (err < 0) {
                  perror("execvp failed with");
              ...
              } else if (pid > 0) { // parent branch must close its own copies
                                    // of pipes so downstream can see EOF
            ...
            redirect_stdout(
                pipefd); // redirect final command's stdout to pipe's write end
            int err = execvp(next_pgm->pgmlist[0], next_pgm->pgmlist);
            if (err < 0) {
              perror("execvp failed with");
            }
            ...
            if (!cmd.background) {
              ...
              foreground_pgid =
                  pid; // give fg the child's pid for sigint_handler ctrl+c
              ...
              waitpid(pid, &status,
                      0); // wait for group leader (first child) and reap it
